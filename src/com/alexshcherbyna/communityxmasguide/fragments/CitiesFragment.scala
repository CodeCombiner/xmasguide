...
import com.alexshcherbyna.communityxmasguide.adapters.CitiesAdapter
import com.alexshcherbyna.communityxmasguide.constants.Constants

import com.alexshcherbyna.communityxmasguide.listeners.{Cities, ListDataRequestSuccessListener, RegisterPersonErrorListener}
...
class CitiesFragment extends SFragment with SprayJsonSupport{

  var lv : SListView = null
  var mAdapter :CitiesAdapter = null
  ...
  override def onCreate(savedInstanceState: Bundle) = {
      ...
      implicit object CitiesFormat extends RootJsonFormat[Cities] {
        ...
        def read(value: JsValue) =
          ...
          value  match {
            ...
            case JsString(cities) => cities.parseJson.asJsObject.getFields("cities")match {
    ...
    val citytr = bundle.getString(Constants.CITIES,"")
    Log.d("CITIES FROM BUNDLE", citytr+"")
    cities = citytr.toJson.convertTo[Cities]
...
var bitmapResource : Int = 0
   ...
   val rootSRL =  new SRelativeLayout {
      ...
      //  val  drawableSearchIcon: Drawable = getResources().getDrawable(R.drawable.search_icon)
        ...
        etSearch.setOnTouchListener(new OnTouchListener() {
          override def onTouch(v: View, event: MotionEvent): Boolean = {
            ...
            if (event.getAction == MotionEvent.ACTION_UP) {
              if (event.getRawX >= (etSearch.getRight - 32 - etSearch.getCompoundDrawables.array(DRAWABLE_RIGHT).getBounds().width )) {
   ...
   //   this += bg
       ...
       mAdapter = new CitiesAdapter(getActivity, getInstance)
     mAdapter.setCities(cities.cities)
...
/*
  def startFragment(selectorType: Int): Unit ={
    ...
    getActivity.asInstanceOf[MainActivity].startFragment(new CitiesFragment(), "ListFragment")
  }
*/
  ...
  def openPlaces(position : Int) = {
   // getActivity.asInstanceOf[MainActivity].startFragment(new ListFragment(), "ListFragment")
    Api.requestCities(new ListDataRequestSuccessListener(getActivity.asInstanceOf[MainActivity], mAdapter),new RegisterPersonErrorListener(getActivity),  getActivity, mAdapter.getItemId(position))

  }

  def cityDerails(position : Int) = {

    val bundle: Bundle  = new Bundle()
     object XmasProtocol2 extends Protocol{}
    import XmasProtocol2._
    bundle.putString(Constants.CITY, mAdapter.getItem(position: Int).asInstanceOf[City].toJson.toString);
    getActivity.asInstanceOf[MainActivity].startFragment(new CityInfoFragment(), bundle)
  }
